...
parser.add_argument(
    ...
    type=int,
    default=50,
    help="number of anomalous cells in the simulation",
)
        ...
        for row in range(ROW_COUNT):
            for col in range(COL_COUNT):
                if self.grid[row][col] == 1:
                    goal = AnomalousGoal(
                        "./resources/red.png",
                        SPRITE_ANOMALOUS_SCALING_GOAL,
                        STAY_ANOMALOUS_ITERATIONS,
                    )
                    ...
                    goal = NormalGoal(
                        "./resources/green.png",
                        SPRITE_NORMAL_SCALING_GOAL,
                        STAY_NORMAL_ITERATIONS,
                    )
                    goal.center_x = (MARGIN + WIDTH) * col + MARGIN + WIDTH // 2
                    goal.center_y = (MARGIN + HEIGHT) * row + MARGIN + HEIGHT // 2
        ...
        for row in range(ROW_COUNT):
            for col in range(COL_COUNT):
                ...
                arcade.draw_rectangle_filled(
                    x, y, WIDTH, HEIGHT, color=arcade.color.WHITE
                )
    ...
    def on_update(self, delta_time):
        ...
        for i in range(len(self.drone_sprite_list) - 1):
            for j in range(i + 1, len(self.drone_sprite_list)):
                if (
                    abs(
                        self.drone_sprite_list[i].center_x
                        - self.drone_sprite_list[j].center_x
                    )
                    < COLLISION_RADIUS / 2
                    and abs(
                        self.drone_sprite_list[i].center_y
                        - self.drone_sprite_list[j].center_y
                    )
                    < COLLISION_RADIUS / 2
                    and total < 0
                ):
        ...
        for drone in self.drone_sprite_list:
            ...
            anomalous_hit_list = arcade.check_for_collision_with_list(
                drone, self.anomalous_goals_sprite_list
            )

            normal_hit_list = arcade.check_for_collision_with_list(
                drone, self.normal_goals_sprite_list
            )
...
def main():
    assert (
        NUM_ANOMALOUS_CELLS <= ROW_COUNT * COL_COUNT
    ), "Number of anomalous cells has to less than total number of cells"
    assert (
        NUM_DRONES < ROW_COUNT * COL_COUNT
    ), "Number of drones has to less than total number of cells"
